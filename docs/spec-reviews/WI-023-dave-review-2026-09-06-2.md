...
reviewed_at: '2026-09-06T19:49:12+01:00'
reviewer: dave
signoff:
  verdict: PROMOTE
  channel: cli
  ...
  frozen_acceptance_criteria: '
    ...
    and **FROZEN**: Dave signed this set on 2026-09-06 and the `## AC Sign-off` fence
    below records it at `ac_hash 583a1b6a293a` (`intent_hash ce7e35a70ea0`, artifact
    `docs/spec-reviews/WI-023-dave-review-2026-09-06.md`, provenance verified). Read
    it as signed, not as editable. One refinement has landed since that signature
    and is disclosed here rather than left to a diff: AC-1''s weak-identity citation
    was re-anchored from the bare `name_validation.py:385-387` (which names an unrelated
    Tier-1 `arrow_connective` branch-table entry) to the symbol `obsidian_schemas/name_validation.py:weak_identity_reason:531-532`,
    which is the clause the criterion was always about and the one D3 already cited
    correctly — a citation repair that changes no promise, and it invalidates the
    frozen hashes (D4b), so a re-sign is owed before `→ ready`. Every `check` is a
    top-level zero-argument `def test_*(` in `tests/` that signals failure by raising.
