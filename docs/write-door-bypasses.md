...
**Second move — 2026-09-05 wrap-up.** The 21 gate-round sections appended after the re-sign (architect 15–18, data-premise 15–20, threat-model 1–3, spec-review 1–3, adversarial 1–3, code review, test & observability) moved to the drawer verbatim; each gate's standing verdict fence stays here in a stub heading. The conductor's *Resume, continued* paragraphs that the driver had buried inside those sections are re-homed under `## Hold state`.
...
> standing verdict carried verbatim at the 2026-09-05 wrap-up archive-split (by hand, WI-267 shape — the tool refuses the 2026-08-11 hand-made drawer as foreign); full round history in `write-door-bypasses-rounds.md`; a fresh gate round supersedes this fence.
...
Run by the conductor session (Dave's "wrap up"), from the lifecycle record rather than a fresh spawn.

### Was the spec accurate?
Yes at the level that matters: the build shipped the eight-arm routing, the gate's output contract,
the placement hoist, the D8 refusal record and the derived wall exactly as the spec pinned them, and
the exit reviewers hand-re-derived the arm/declaration/placement triples against the post-build bytes
and found them identical. The one drift was in the INSTRUMENT, not the design: three rules (merge idiom,
association/placement, declaration expression) were each stated in several registers that disagreed,
and rounds 16–18 found them one per round. Each was a one-clause repair in unsigned text; none moved a
signed criterion or a ruling. The AC set signed at 10:11 stood unchanged through to `done`.

### Edge cases that surprised us
- **The AC battery's interpreter.** The conveyor runs `kind: test` checks under the advancer's
  interpreter unless the driver is given `--ac-python`; every criterion went RED with
  `ModuleNotFoundError: pydantic` against a green floor. First item here whose checks execute the
  package rather than AST-sweep it. Bridged in-project (`tests/ac_interpreter.py`); the launcher fix is
  workshop's (taken into the agent-friendly CLI mint).
- **`normalize_phone` collisions were a real deletion** (G12: five live notes holding one number as
  `447…` and `+447…`); Dave ruled the E.164 spelling wins (ruling 4). Not in the spec's Edge Cases until
  round 18's data audit sized it.
- **`_writeback_identifier`'s in-memory divergence at D4**: after the gate dedupes, the committed file
  holds one entry while the caller's `person.phones` still holds two; disk converges on the next save.
  Named by data-premise round 19 as the architect's seat, never raised there. Follow-up candidate.
- **Two exit gates epoch-skipped** (intent-check, verify-declaration) because the item was created
  2026-07-05, one day before their epochs. The build was reviewed by code-reviewer and
  test-observability-checker only; Dave may want the intent check run by hand.

### What would have shortened the build?
1. **`--ac-python` on the launch** — one flag, would have saved a build attempt and the bridge.
2. **The zero-blocking-REVISE treadmill**: rounds 18 and 19 each cost a producer pass plus the objecting
   gate (2 spawns, a relaunch, and a hand fold) to learn that nothing blocked, because the driver routes
   on the verdict word alone. Recorded as a defect specimen; workshop is minting the lane.
3. **Register-disagreement rules**: a spec-writer instruction — *state each rule ONCE and cite it from the
   other registers* — would have removed rounds 16–18's findings before they were written.
4. **Conductor hygiene**: one spawn and a ledger restore were lost to the conductor writing the live tree
   during a leg (fence-tamper). Rule now in the conductor scars: ledger/escalation/commit writes only
   between legs.

### Did the build serve the original intent, or the spec's drift of it?
The intent — *no door into the vault through which an unvalidated name or unnormalized address can pass;
one RFC 2822 parse authority; an invariant test per closed door* — is what shipped: every frontmatter arm
routes through one gate, the splitter is single-homed, and the wall is derived so a ninth door is red
without editing it. The one intent-level narrowing was deliberate and ruled: rule (ii) refuses an
UNDECLARED name write outright rather than validating it (Dave's ruling 2), which is stricter than the
intent, not weaker. No gap to surface.

### Recommended follow-ups
- **workshop** (already taken by the factory session): launcher defaults `--ac-python`/`--test-command`
  from the project declaration; zero-blocking REVISE routes as PROMOTE-with-notes and leaves the
  treadmill streak; consumer lock-sentinel gitignore floor check.
- **workshop, new**: `archive-split.py` refuses a hand-made drawer as `foreign-drawer`, so this doc's two
  splits are both by hand — an "adopt this drawer" path (or a documented drawer mark) would let the tool
  take over.
- **obsidian-schemas, candidate items**: (a) the D4 in-memory `person.phones` divergence after gate
  dedupe; (b) if Dave's "standard iso format" meant rewriting ALL stored phones to E.164 rather than the
  duplicate-winner rule applied here, that is a display-form normalization outside AC-4 and its own item.
- **spec-writer bar**: "state each design rule once; every other register cites it" (the round 16–18
  class).

### Build-earned lesson (LESSONS.html bar)
One candidate clears the bar and is PROPOSED, not written: **never write a driven tree while a leg is
running** — the fence-tamper guard attributes any live delta in a gate window to the gate, reverts the
file and STOPs the drive. Scar: 2026-09-05, twice on one machine in one afternoon (workshop lost 49 ledger
events; obsidian-schemas lost a spawn and needed a restore from evidence bytes). Rule: ledger records,
escalation answers, mints and commits only between legs, after `live_drives(project=…)` is empty. Dave
decides whether it goes in LESSONS.html.
