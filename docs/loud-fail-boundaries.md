...
Round 22 (fence below) confirmed the AC-7 split and its clauses correct as typed — and found the mechanism's real defect: per-derivation cardinality requires discriminating derivations by shape, and three of the six share one shape while two differ only semantically, so the assertion was unimplementable against a correct tree. The fold inverts the detection: catch what every copy must DO (touch the ast module — single-homed, both marker forms planted as negatives) rather than what it MEANS, and assert completeness by the shared module's six named exports, where distinctness is decidable by import. Capability uniqueness implies predicate uniqueness; nothing semantic is ever shape-detected. Dave ruled on a one-line ask; nothing else moved.
