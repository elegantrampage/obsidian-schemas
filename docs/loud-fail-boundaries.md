...
**Given** a person note someone hand-edited so the `## Timeline` heading is gone, **when** the exocortex meeting sync calls `append_to_timeline` to record yesterday's meeting, **then** it fails loudly — instead of returning the same `False` it returns when the entry was already there, so the sync believes it deduplicated while the meeting record was silently thrown away. The note was created by this package's own template, which always writes a `## Timeline` section — its absence means the note is damaged, not that there was nothing to do.
