...
Round 24 (fence below) confirmed the capability-level restatement whole and found its one honest hole: the sufficiency premise quantified over six derivations but held for five — the file-set walk touches no syntax, so the simplest derivation to privately reimplement was the one the ast marker could not see. The fold is the smallest of the arc: sufficiency restated per member, and the sixth member given its own marker — .py-targeted filesystem enumeration, single-homed, planted negatives per form, with AC-3's .md fixture vaults explicitly outside the marker's universe. Two markers now jointly cover all six by what a copy must do. Dave ruled on a one-line ask; nothing else moved.
